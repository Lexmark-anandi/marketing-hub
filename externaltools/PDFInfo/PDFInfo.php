...
class PDFInfo
{
    ...
    private function loadOutput()
    {
        ...
        exec("$cmd -box $file", $output, $returnVar);
    ...
    private function parseOutput()
    {
        ...
        $this->mediaBox = $this->parseBox('MediaBox');
        $this->cropBox = $this->parseBox('CropBox');
        $this->bleedBox = $this->parseBox('BleedBox');
        $this->trimBox = $this->parseBox('TrimBox');
        $this->artBox = $this->parseBox('ArtBox');
    }
    ...
    private function parseBox($attribute)
    {
        // Iterate through lines
        $result = null;
        foreach($this->output as $op)
        {
            ...
            if(preg_match("/" . $attribute . ":\s*(.+)/i", $op, $matches) === 1)
            {
				$strTmp = $matches[1];
				$aTmp = explode(' ', $strTmp);
				$result = array_values(array_filter($aTmp, function($value) { return $value !== ''; }));
                break;
            }
        }
