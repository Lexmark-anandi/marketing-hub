...
foreach($rows as $row){
	...
	$row = setValuesRead($row, $aFields, $CONFIG['page']['moduls'][$varSQL['modul']]['activeCountry'], $CONFIG['page']['moduls'][$varSQL['modul']]['activeLanguage'], $CONFIG['page']['moduls'][$varSQL['modul']]['activeDevice']);

	$contentRaw->rows[$i] = $row + $rowExp;
	$contentRaw->rows[$i]['id'] = $row['id_clid'];

	foreach($row as $k=>$v){
		$row[$k] = '<div class="gridCellFold">' . $v . '</div>';
	}
	
	$row['id_clid'] = strip_tags($row['id_clid']);
	
	$content->rows[$i] = $row;
	$content->rows[$i]['id'] = $row['id_clid'];
