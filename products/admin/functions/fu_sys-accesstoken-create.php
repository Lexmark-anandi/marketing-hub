...
function createAccesstoken($args=array()){
	...
	setcookie('csrf', $csrfSignature, 0, '/'.$CONFIG['system']['directorySystem'].$CONFIG['system']['pathAdmin'], $_SERVER['HTTP_HOST'], $CONFIG['system']['cookie_secure'], false);
	...
	$token .= '.' . $signature;
	setcookie('access', $token, 0, '/'.$CONFIG['system']['directorySystem'].$CONFIG['system']['pathAdmin'], $_SERVER['HTTP_HOST'], $CONFIG['system']['cookie_secure'], true);
}
