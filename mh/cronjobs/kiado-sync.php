...
foreach($rows1 as $row1){
	...
	if($code != '200'){
		...
		$query2 = $CONFIG['dbconn'][0]->prepare('
											...
											WHERE ' . $CONFIG['db'][0]['prefix'] . '_kiadocodes_loc.del = (:nultime)
												AND ' . $CONFIG['db'][0]['prefix'] . '_kiadocodes_loc.id_count = (:id_count)
												AND ' . $CONFIG['db'][0]['prefix'] . '_kiadocodes_loc.id_lang = (:id_lang)
												AND ' . $CONFIG['db'][0]['prefix'] . '_kiadocodes_loc.id_kcid = (:id_kcid)
											');
		$query2->bindValue(':link', $rowsM[0]['link'], PDO::PARAM_STR);
		$query2->bindValue(':id_count', $row1[0]['id_count'], PDO::PARAM_INT);
		$query2->bindValue(':id_lang', $row1[0]['id_lang'], PDO::PARAM_INT);
		$query2->bindValue(':nultime', '0000-00-00 00:00:00', PDO::PARAM_STR);
		$query2->bindValue(':id_kcid', $row1[0]['id_kcid'], PDO::PARAM_INT);
		$query2->execute();
		...
		$query2 = $CONFIG['dbconn'][0]->prepare('
											...
											WHERE ' . $CONFIG['db'][0]['prefix'] . '_kiadocodes_uni.del = (:nultime)
												AND ' . $CONFIG['db'][0]['prefix'] . '_kiadocodes_uni.id_count = (:id_count)
		...
		$query2 = $CONFIG['dbconn'][0]->prepare('
											...
											WHERE ' . $CONFIG['db'][0]['prefix'] . 'sys_mediafiles_loc.del = (:nultime)
		...
		$query2->bindValue(':id_lang', $row1[0]['id_lang'], PDO::PARAM_INT);
		$query2->bindValue(':nultime', '0000-00-00 00:00:00', PDO::PARAM_STR);
		$query2->bindValue(':id_mid', $row1[0]['id_mid'], PDO::PARAM_INT);
		$query2->execute();
		...
		$query2 = $CONFIG['dbconn'][0]->prepare('
											...
											WHERE ' . $CONFIG['db'][0]['prefix'] . 'sys_mediafiles_uni.del = (:nultime)
												AND ' . $CONFIG['db'][0]['prefix'] . 'sys_mediafiles_uni.id_count = (:id_count)
