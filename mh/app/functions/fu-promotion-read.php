...
if($varSQL['id_asid'] == 0 || $varSQL['id_asid'] == ''){
	...
	$queryCo->bindValue(':id_promid', $varSQL['id_promid'], PDO::PARAM_INT);
	...
	foreach($rowsCo as $rowCo){
		$queryCo2 = $CONFIG['dbconn'][0]->prepare('
											...
											(id_asid, id_count, id_lang, id_dev, id_cl, restricted_all, id_promid, id_tempid, id_pcid, id_ppid, title, components)
											VALUES
											(:id_asid, :id_count, :id_lang, :id_dev, :id_cl, :restricted_all, :id_promid, :id_tempid, :id_pcid, :id_ppid, :title, :components)
											');
		...
		$queryCo2->bindValue(':title', $rowCo['title_promotion'], PDO::PARAM_INT);
	...
	$queryCo = $CONFIG['dbconn'][0]->prepare('
										INSERT INTO ' . $CONFIG['db'][0]['prefix'] . '_assets_tmp
										(id_asid, id_count, id_lang, id_dev, id_cl, restricted_all, id_promid, id_tempid, id_pcid, id_ppid, title, components, thumbnail)
										SELECT
										id_asid, id_count, id_lang, id_dev, id_cl, restricted_all, id_promid, id_tempid, id_pcid, id_ppid, title, components, thumbnail
...
$query = $CONFIG['dbconn'][0]->prepare('
									...
									INNER JOIN ' . $CONFIG['db'][0]['prefix'] . '_templates_uni
										ON ' . $CONFIG['db'][0]['prefix'] . '_templates_uni.id_count = (:id_count)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templates_uni.id_lang = (:id_lang)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templates_uni.del = (:nultime)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templates_uni.id_promid = ' . $CONFIG['db'][0]['prefix'] . '_promotions_uni.id_promid
...
$query->bindValue(':id_promid', $varSQL['id_promid'], PDO::PARAM_INT);
...
//	$fileThumbnail = str_pad($row['id_tempid'], 6 ,'0', STR_PAD_LEFT) . '-' . $CONFIG['user']['id_countid'] . '-' . $CONFIG['user']['id_langid'] . '-' . md5($row['id_tempid'] . '_template') . '-1.png';
//	$out['templates'][$row['id_tempid']] = array('src' => $dirTarget . 'templates_thumbnails/' . $fileThumbnail, 'temp' => $row['id_tempid'], 'pagelabel' => $row['title_template']);
}
