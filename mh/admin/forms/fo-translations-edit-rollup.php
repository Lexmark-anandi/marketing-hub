...
foreach($CONFIG['user']['countries'] as $id_countid => $aCountry){
	foreach($aCountry['languages'] as $id_langid){
		$fieldUpload .= '<div class="formRow formRowHidden " data-fieldvariation="'.$id_countid.'_'.$id_langid.'_x##file_original"data-fieldname="file_original">';
		$fieldUpload .= '<div class="formLabel">';
		$fieldUpload .= '<label for="logo_'.$modulpath.'">' . $TEXT['fileupload'] . '</label>';
		$fieldUpload .= '</div>';
		$fieldUpload .= '<div class="formField">';
		$fieldUpload .= '<input type="file" name="'.$id_countid.'_'.$id_langid.'_x_file_original" id="'.$id_countid.'_'.$id_langid.'_x_file_original_'.$modulpath.'" class="textfield fileupload " value="" data-checkfunction="" data-checkmessage="" data-fieldname="file_original"  data-allowedtypes="pdf" data-target="printads" data-checksync="device">';
		$fieldUpload .= '</div>';
		$fieldUpload .= '</div>';
	} 
}
