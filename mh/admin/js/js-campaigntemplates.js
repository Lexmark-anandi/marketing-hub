(function () {
    ...
    window['f_##modul_name##']['closeTemplates'] = function (obj) { 
    ...
    window['f_##modul_name##']['rowAdd'] = function (obj, el) { 
		...
		obj.id_data = 0;
		obj.id_data_parent = $('#form_' + $(el).closest('.modul').attr('data-modulpath') + ' .formLeft [name="id_data"]').val();
		obj.splitForm = 0;
		...
		$('.modul .formRight').css('bottom', '0px'); 
		loadForm(obj, el);
	};
    ...
    window['f_##modul_name##']['cbLoadDataSuccess'] = function (obj) {
		$('#form_' + obj.modulpath + ' .formLeft [name="caid"]').val($('#modul_' + obj.modulpath + ' .formLeft select[name="id_caid"] option:selected').val());
		$('#form_' + obj.modulpath + ' .formLeft [name="contentselection"]').val($('#modul_' + obj.modulpath + ' .formLeft input[name="contentselect"]:checked').val());
		
		var actStep = $('#modul_' + obj.modulpath + ' .formLeft .fieldsetActive').attr('data-step') * 1;
		(actStep == $('#modul_' + obj.modulpath + ' .formLeft .fieldset[data-step]').length) ? $('#form_' + obj.modulpath + ' .formLeft .nextStep').html(objText.finish) : $('#form_' + obj.modulpath + ' .formLeft .nextStep').html(objText.NextStep);
		...
		if(actStep == 2){
			$('.formRight .fileupload').fileupload('destroy');
...
//		}
		...
		if(objData.id_caid == 9){
			$('#form_' + obj.modulpath + ' .formLeft #formPoster').css('display', 'block');
		}
    ...
    window['f_##modul_name##']['loadCountriesKiado'] = function (obj) { 
		...
		if(kiado != ''){
			$('#form_' + obj.modulpath + ' .formLeft .formRowCountriesKiado .countryTableRow').each(function(){
				...
				$.ajax({  
					url: objSystem.directoryInstallation + objSystem.pathFormsAdmin + 'fo-templates-countries-kiado.php', 
					data: data,    
					type: 'post',          
					cache: false,  
					headers: {
...
//						
						...
						if($('#form_' + obj.modulpath + ' .formLeft [name="caid"]').val() != 3){
...
//								}
    ...
    window['f_##modul_name##']['loadPageComponents'] = function (obj) { 
		...
		if(objComponents.pages['page_' + compPageId] != undefined){
			for(key in objComponents.pages['page_' + compPageId]){
				...
				if(objComponents.pages['page_' + compPageId][key]['fixed'] == 1 || $('#form_' + obj.modulpath + ' .formLeft [name="caid"]').val() == 3){
    ...
    window['f_##modul_name##']['resizeStopPlaceholder'] = function (obj, event, ui, el) { 
		var compPageId = $('#modul_' + obj.modulpath + ' .formLeft .formComponentThumbOuterActive').attr('data-pageid');
//		var compWidth = ui.size.width;
//		var compHeight = ui.size.height;
		var compId = $(el).attr('id');
		var compWidth = ui.helper.outerWidth();
		var compHeight = ui.helper.outerHeight();

		var compWidth = $(el).innerWidth();
		var compHeight = $(el).innerHeight();
...
//			}
    ...
    window['f_##modul_name##']['goNextStep'] = function (obj) {
		...
		if(actStep == $('#modul_' + obj.modulpath + ' .formLeft .fieldset[data-step]').length){
			window['f_##modul_name##']['sendForm'](obj, 'close');
			$('.modul .formRight').css('bottom', $('.modul .formLeft .formFooter').outerHeight(true) + 'px');
			
		}else if($('#modul_' + obj.modulpath + ' .formLeft .fieldset[data-step="' + newStep + '"]').length > 0){ 
			...
			if(actStep == 1){
				checkRequired(obj, '#modul_' + obj.modulpath + ' .formLeft .textfieldTitle');
				checkSelectNotEqual0(obj, '#modul_' + obj.modulpath + ' .formLeft select[name="id_caid"]');
				
				var cat = $('#modul_' + obj.modulpath + ' .formLeft select[name="id_caid"] option:selected').val();
				if(cat == 4 || cat == 5){
					checkRadioRequired(obj, '#modul_' + obj.modulpath + ' .formLeft input[name="contentselect"]');
				}
			}
			// ##################################
