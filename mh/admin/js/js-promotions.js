(function () {
    ...
    window['f_##modul_name##']['cbLoadFormSuccess'] = function (obj) { 
		...
		$('#form_' + obj.modulpath + ' .formLeft .templatePublish').on('click', function(){
			...
			if(type == 'translation'){
				...
				objDialog.formtext = objText.TemplatePublishCheckPromo;
    ...
    window['f_##modul_name##']['cbLoadDataSuccess'] = function (obj) {
		...
		if(countries.length > 0){
			...
			$.ajax({  
				url: objSystem.directoryInstallation + objSystem.pathFormsAdmin + 'fo-promotions-partner.php', 
				data: data,    
				type: 'post',          
				cache: false,  
				headers: {
...
//		}
		...
		if(actStep == 6){
			...
			var partner = '';
			$('.selectPartner option:selected').each(function(){
				partner += '<div>' + $(this).text() + '</div>';
			});
			$('.overviewPartner').html(partner);
			...
			$.ajax({  
				url: objSystem.directoryInstallation + objSystem.pathFunctionsAdmin + 'fu-promotions-templateslist.php', 
				data: data,    
				type: 'post',          
				cache: false,  
				headers: {
    ...
    window['f_##modul_name##']['checkPublish'] = function (obj) { 
		...
		if(objUser.right != 4){
			...
			var data = 'promo=' + objData.id_promid;
			
			$.ajax({  
				url: objSystem.directoryInstallation + objSystem.pathFunctionsAdmin + 'fu-promotions-trans-check.php', 
				data: data,    
				type: 'post',          
				cache: false,  
				headers: {
    ...
    window['f_##modul_name##']['templatePublishDo'] = function (obj, el) { 
		...
		$.ajax({  
			url: objSystem.directoryInstallation + objSystem.pathFunctionsAdmin + 'fu-promotions-publish.php', 
			data: data,    
			type: 'post',          
			cache: false,  
			headers: {
			...
			success: function (result, status, jqXHR) {
				...
				$.ajax({  
					url: objSystem.directoryInstallation + objSystem.pathFunctionsAdmin + 'fu-promotions-preview-create.php', 
					data: data,    
					type: 'post',          
					cache: false,  
					headers: {
