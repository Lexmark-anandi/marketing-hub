(function () {
    window['f_##modul_name##']['dashboardwidgettemplatespercountry_Ready'] = function (obj, data, el) { 
		var labels = [];
		for(var key in data){
...
//						}
    ...
    window['f_##modul_name##']['dashboardwidgettemplatespercountry_ExportExcel'] = function (obj, el) { 
		waiting('body');
		var data = '';
		
		$.ajax({  
			url: objSystem.directoryInstallation + objSystem.pathFunctionsAdmin + 'fu-dashboard-widget-templates-per-country-export.php',    
			type: 'post',          
			data: data,       
			cache: false,
			headers: {
