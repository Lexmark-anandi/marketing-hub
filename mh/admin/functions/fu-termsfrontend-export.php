...
$queryD = $CONFIG['dbconn'][0]->prepare('
									...
									ORDER BY ' . $CONFIG['db'][0]['prefix'] . 'sys_terms_fe_uni.id_tfeid
									');
...
foreach($rowsD as $rowD){
	$c = 0;
	$id = $rowD['id_tfe_data'] . '.' . $rowD['id_tfeid'] . '.' . $rowD['id_count'] . '.' . $rowD['id_lang'] . '.' . $rowD['id_dev'] . '.' . $rowD['id_cl'];
...
$con['filename'] = str_replace(' ', '_', $TEXT['Terms Frontend']) . $fileCode . '_' . $now . '.xlsx';
