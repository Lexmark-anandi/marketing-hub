...
if($CONFIG['page']['id_data'] == 0){
	include_once($CONFIG['system']['directoryRoot'] . $CONFIG['system']['pathFunctionsAdmin'] . 'fu-campaigns-read-new.php');
	...
	$tableLoc = '_campaigns_loc';
	$tableExt = '_campaigns_ext';
	
	$primarykey = 'id_campid';
		...
		if($num == 0){
			...
			if($numD > 0){
				...
				$queryN = $CONFIG['dbconn'][0]->prepare('
													SELECT 
														' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_count2lang
				...
				$queryN = $CONFIG['dbconn'][0]->prepare('
													SELECT 
														' . $CONFIG['db'][0]['prefix'] . '_campaigns2products_.id_pid
