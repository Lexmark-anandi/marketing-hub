...
$tempid = (isset($varSQL['reminder'])) ? $varSQL['reminder'] : $CONFIG['page']['id_data'];
...
$query->bindValue(':id_tempid', $tempid, PDO::PARAM_STR);
if($CONFIG['settings']['formCountry'] != 0){ 
...
$queryT->bindValue(':id_tempid', $tempid, PDO::PARAM_INT);
$queryT->execute();
$rowsT = $queryT->fetchAll(PDO::FETCH_ASSOC);
$numT = $queryT->rowCount();

$CATEGORY = $rowsT[0]['category'];
...
$queryC = $CONFIG['dbconn'][0]->prepare('
									...
									INNER JOIN  ' . $CONFIG['db'][0]['prefix'] . 'system_user2countries2languages
										ON ' . $CONFIG['db'][0]['prefix'] . '_templates2countries_.id_count2lang = ' . $CONFIG['db'][0]['prefix'] . 'system_user2countries2languages.id_count2lang
...
$queryC->bindValue(':id_tempid', $tempid, PDO::PARAM_INT);
...
foreach($rowsC as $rowC){
	...
	$queryL = $CONFIG['dbconn'][0]->prepare('
										SELECT ' . $CONFIG['db'][0]['prefix'] . '_templates2countries_.id_countid,
											' . $CONFIG['db'][0]['prefix'] . '_templates2countries_.id_langid,
										...
										INNER JOIN  ' . $CONFIG['db'][0]['prefix'] . 'sys_languages_uni
											ON ' . $CONFIG['db'][0]['prefix'] . '_templates2countries_.id_langid = ' . $CONFIG['db'][0]['prefix'] . 'sys_languages_uni.id_langid
	
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_templates2countries_.del = (:nultime)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templates2countries_.id_tempid = (:id_tempid)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templates2countries_.id_countid IN (' . implode(',', $aCountUser) . ')
											AND ' . $CONFIG['db'][0]['prefix'] . '_templates2countries_.published_at = (:nultime)
	...
	$queryL->bindValue(':id_tempid', $tempid, PDO::PARAM_INT);
	$queryL->execute();
	...
	foreach($rowsL as $rowL){
		...
		$TEMPLATE_DEEPLINK .= $rowL['country'] . ' / ' . $rowL['language'] . ':<br><a href="' . $link . '">' . $link . '</a><br><br>';
	}
	...
	$mailing['body'] = str_replace('##CATEGORY##', $CATEGORY, $mailing['body']);
