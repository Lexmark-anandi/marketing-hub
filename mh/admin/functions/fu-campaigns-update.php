...
$aArgsSave['table'] = $CONFIG['db'][0]['prefix'] . '_campaigns_';
$aArgsSave['primarykey'] = 'id_campid';
...
array_push($aArgsSave['preview_thumbnail'], 'bsd_only');

$aArgsSave['excludeUpdateUni'] = array();
$aArgsSave['excludeUpdateUni']['id_campid'] = array('');
...
foreach($rows as $row){
	...
	if($CONFIG['user']['specifications'][14] == 8 && $variation == 'master' && $CONFIG['settings']['formCountry'] == 0 && $CONFIG['settings']['formLanguage'] == 0 && $CONFIG['settings']['formDevice'] == 0){
		...
		$queryP1t = $CONFIG['dbconn'][0]->prepare('
											SELECT ' . $CONFIG['db'][0]['prefix'] . '_campaigns_loc.id_campid,
												' . $CONFIG['db'][0]['prefix'] . '_campaigns_loc.create_from
											FROM ' . $CONFIG['db'][0]['prefix'] . '_campaigns_loc 
											WHERE ' . $CONFIG['db'][0]['prefix'] . '_campaigns_loc.id_campid = (:id_campid)
											LIMIT 1
											');
		$queryP1t->bindValue(':id_campid', $aArgsSave['id_data'], PDO::PARAM_INT);
		$queryP1t->execute();
	...
	$queryN = $CONFIG['dbconn'][0]->prepare('
										UPDATE ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_ SET
											del = (:now)
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_campid = (:id)
	...
	if(isset($aArgsSave['aData']['country'])){
		...
		foreach($aArgsSave['aData']['country'] as $val){
			...
			$queryN = $CONFIG['dbconn'][0]->prepare('
												INSERT INTO ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_
												(id_count, id_lang, id_dev, id_cl, id_campid, id_countid, id_langid, id_count2lang, create_at, create_from, change_from)
												VALUES
												(:id_count, :id_lang, :id_dev, :id_cl, :id_campid, :id_countid, :id_langid, :id_count2lang, :now, :create_from, :create_from)
												ON DUPLICATE KEY UPDATE 
												del = (:nultime)
												');
			...
			$queryN->bindValue(':id_campid', $aArgsSave['id_data'], PDO::PARAM_INT);
	...
	if($row['id_count'] == $CONFIG['settings']['formCountry'] && $row['id_lang'] == $CONFIG['settings']['formLanguage'] && $row['id_dev'] == $CONFIG['settings']['formDevice']){
		$queryN = $CONFIG['dbconn'][0]->prepare('
											UPDATE ' . $CONFIG['db'][0]['prefix'] . '_campaigns2products_ SET
												del = (:now)
											WHERE ' . $CONFIG['db'][0]['prefix'] . '_campaigns2products_.id_campid = (:id)
											');
		$queryN->bindValue(':id', $aArgsSave['id_data'], PDO::PARAM_INT);
		$queryN->bindValue(':now', $now, PDO::PARAM_STR);
		$queryN->execute();
		...
		if(isset($aArgsSave['aData']['selectassign_products'])){
			...
			foreach($aArgsSave['aData']['selectassign_products'] as $val){
				$queryN = $CONFIG['dbconn'][0]->prepare('
													INSERT INTO ' . $CONFIG['db'][0]['prefix'] . '_campaigns2products_
													(id_count, id_lang, id_dev, id_cl, id_campid, id_pid, create_at, create_from, change_from)
													VALUES
													(:id_count, :id_lang, :id_dev, :id_cl, :id_campid, :id_pid, :now, :create_from, :create_from)
													ON DUPLICATE KEY UPDATE 
													del = (:nultime)
													');
				$queryN->bindValue(':id_count', 0, PDO::PARAM_INT);
