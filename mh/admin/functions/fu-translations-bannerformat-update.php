...
if($numMo == 0){
	$queryMo = $CONFIG['dbconn'][0]->prepare('
										SELECT ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_loc.width,
											' . $CONFIG['db'][0]['prefix'] . '_bannerformats_loc.height
										FROM ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_loc 
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_loc.id_bfid = (:id_bfid)
											AND ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_loc.id_count = (:id_count)
											AND ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_loc.id_lang = (:id_lang)
										');
	$queryMo->bindValue(':id_bfid', $varSQL['bfid'], PDO::PARAM_INT);
	$queryMo->bindValue(':id_count', 0, PDO::PARAM_INT);
	$queryMo->bindValue(':id_lang', 0, PDO::PARAM_INT);
	$queryMo->execute();
	$rowsMo = $queryMo->fetchAll(PDO::FETCH_ASSOC);
...
$aArgs['data'] = array();
$aArgs['data']['id_bfid'] = $varSQL['bfid'];
$aArgs['data']['bannername'] = $varSQL['bannername'];
$aArgs['data']['width'] = $rowsMo[0]['width'];
$aArgs['data']['height'] = $rowsMo[0]['height'];
$aArgs['data']['rank'] = 0;
$aArgs['data']['active'] = 1;
