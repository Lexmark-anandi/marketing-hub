...
foreach($sheetData as $key=>$val){
	if($key > 1){
		...
		$qry = 'INSERT INTO ' . $CONFIG['db'][0]['prefix'] . 'sys_terms_fe_loc
					(id_tfeid, id_count, id_lang, id_dev, id_cl, create_at, create_from, change_from, term)
				VALUES
					(:id_tfeid, :id_count, :id_lang, :id_dev, :id_cl, :now, :create_from, :create_from, :term)
				ON DUPLICATE KEY UPDATE 
					term = (:term)
				';
		$query2 = $CONFIG['dbconn'][0]->prepare($qry);
		$query2->bindValue(':id_tfeid', $id, PDO::PARAM_INT);
		...
		$query2 = $CONFIG['dbconn'][0]->prepare('
											UPDATE ' . $CONFIG['db'][0]['prefix'] . 'sys_terms_fe_uni SET
												term = (:term)
											WHERE ' . $CONFIG['db'][0]['prefix'] . 'sys_terms_fe_uni.id_tfe_data = (:id_tfe_data)
												AND ' . $CONFIG['db'][0]['prefix'] . 'sys_terms_fe_uni.id_tfeid = (:id_tfeid)
												AND ' . $CONFIG['db'][0]['prefix'] . 'sys_terms_fe_uni.id_count = (:id_count)
												AND ' . $CONFIG['db'][0]['prefix'] . 'sys_terms_fe_uni.id_lang = (:id_lang)
												AND ' . $CONFIG['db'][0]['prefix'] . 'sys_terms_fe_uni.id_dev = (:id_dev)
												AND ' . $CONFIG['db'][0]['prefix'] . 'sys_terms_fe_uni.id_cl = (:id_cl)
											LIMIT 1
											');
		$query2->bindValue(':id_tfe_data', $id_data, PDO::PARAM_INT);
		$query2->bindValue(':id_tfeid', $id, PDO::PARAM_INT);
		$query2->bindValue(':id_count', $id_count, PDO::PARAM_INT);
		$query2->bindValue(':id_lang', $id_lang, PDO::PARAM_INT);
		$query2->bindValue(':id_dev', $id_dev, PDO::PARAM_INT);
