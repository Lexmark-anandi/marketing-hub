	...
	if($num == 0){
		...
		if($numD > 0){
			...
			for($i=1; $i <= $pages; $i++){
				...
				$aArgs['fields']['file'][$bannername] = array('pf' => '0', 'thumbnail' => '0', 'functions' => array(22), 'gridheight' => 'fold', 'type' => 'single');

				$rowsD[0][$bannername] = array();
				//$rowsD[0]['id_bfid'] = $rowB['id'];
			}
...
//															AND ' . $CONFIG['db'][0]['prefix'] . '_templatespages_uni.page = (:page)
//														');
...
//					$queryN->bindValue(':id_bfid', $rowB['id'], PDO::PARAM_INT);
//					$queryN->bindValue(':page', $i, PDO::PARAM_INT);
//					$queryN->execute();
//					$rowsN = $queryN->fetchAll(PDO::FETCH_ASSOC);
//					$numN = $queryN->rowCount();
//					if($numN > 0){
//						$rowsD[0]['id_tpid'] = $rowsN[0]['id_tpid'];
//						$rowsD[0][$bannername] = array($rowsN[0]['file_original']);
//					}
//				}
//			}
