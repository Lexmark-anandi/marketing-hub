...
$query->bindValue(':id_campid', $promid, PDO::PARAM_STR);
if($CONFIG['settings']['formCountry'] != 0){
...
$query = $CONFIG['dbconn'][0]->prepare('
									...
									WHERE ' . $CONFIG['db'][0]['prefix'] . '_campaigns_ext.id_campid = (:id_campid)
										AND ' . $CONFIG['db'][0]['prefix'] . '_campaigns_ext.published_at = (:nultime)
										' . $cond . '
									');
$query->bindValue(':nultime', '0000-00-00 00:00:00', PDO::PARAM_STR);
$query->bindValue(':now', $now, PDO::PARAM_STR);
...
$query = $CONFIG['dbconn'][0]->prepare('
									...
									WHERE ' . $CONFIG['db'][0]['prefix'] . '_campaigns_loc.id_campid = (:id_campid)
										AND ' . $CONFIG['db'][0]['prefix'] . '_campaigns_loc.published_at = (:nultime)
										' . $cond . '
									');
$query->bindValue(':nultime', '0000-00-00 00:00:00', PDO::PARAM_STR);
$query->bindValue(':now', $now, PDO::PARAM_STR);
...
$query = $CONFIG['dbconn'][0]->prepare('
									...
									WHERE ' . $CONFIG['db'][0]['prefix'] . '_campaigns_res.id_campid = (:id_campid)
										AND ' . $CONFIG['db'][0]['prefix'] . '_campaigns_res.published_at = (:nultime)
										' . $cond . '
									');
$query->bindValue(':nultime', '0000-00-00 00:00:00', PDO::PARAM_STR);
$query->bindValue(':now', $now, PDO::PARAM_STR);
...
$queryT->bindValue(':id_campid', $promid, PDO::PARAM_INT);
$queryT->execute();
$rowsT = $queryT->fetchAll(PDO::FETCH_ASSOC);
$numT = $queryT->rowCount();

$TEMPLATEID = $rowsT[0]['id_campid'];
...
$queryC = $CONFIG['dbconn'][0]->prepare('
									SELECT ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_countid,
									...
									FROM ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_ 
										
									INNER JOIN  ' . $CONFIG['db'][0]['prefix'] . 'system_user2countries2languages
										ON ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_count2lang = ' . $CONFIG['db'][0]['prefix'] . 'system_user2countries2languages.id_count2lang
									...
									WHERE ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.del = (:nultime)
...
$queryC->bindValue(':id_campid', $promid, PDO::PARAM_INT);
...
foreach($rowsC as $rowC){
	...
	$queryL = $CONFIG['dbconn'][0]->prepare('
										SELECT ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_countid,
											' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_langid,
										...
										INNER JOIN  ' . $CONFIG['db'][0]['prefix'] . '_templates_uni
											ON ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_campid = ' . $CONFIG['db'][0]['prefix'] . '_templates_uni.id_campid
												AND ' . $CONFIG['db'][0]['prefix'] . '_templates_uni.id_count = ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_countid
												AND ' . $CONFIG['db'][0]['prefix'] . '_templates_uni.id_lang = ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_langid
										...
										INNER JOIN  ' . $CONFIG['db'][0]['prefix'] . 'sys_languages_uni
											ON ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_langid = ' . $CONFIG['db'][0]['prefix'] . 'sys_languages_uni.id_langid
	
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.del = (:nultime)
											AND ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_campid = (:id_campid)
											AND ' . $CONFIG['db'][0]['prefix'] . '_campaigns2countries_.id_countid IN (' . implode(',', $aCountUser) . ')
											AND ' . $CONFIG['db'][0]['prefix'] . '_templates_uni.published_at = (:nultime)
	...
	$queryL->bindValue(':id_campid', $promid, PDO::PARAM_INT);
	$queryL->execute();
...
//			print_r($arr);
	...
	$mailing['body'] = str_replace('##CATEGORY##', $TEXT['Campaigns'], $mailing['body']);
