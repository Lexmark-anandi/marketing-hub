...
foreach($aLocalVersions as $aVersion){
	...
	if($aVersion[0] == 0){
		...
		$query = $CONFIG['dbconn'][0]->prepare('
											...
											WHERE ' . $CONFIG['db'][0]['prefix'] . 'system_countries.id_sys_count = (:count)
	...
	if($aVersion[1] == 0){
		...
		$query = $CONFIG['dbconn'][0]->prepare('
											...
											WHERE ' . $CONFIG['db'][0]['prefix'] . 'system_languages.id_sys_lang = (:lang)
	...
	if($aVersion[2] == 0){
		...
		$query = $CONFIG['dbconn'][0]->prepare('
											...
											WHERE ' . $CONFIG['db'][0]['prefix'] . 'system_devices.id_sys_dev = (:dev)
												AND ' . $CONFIG['db'][0]['prefix'] . 'system_devices.del = (:nultime)
											LIMIT 1
											');
	...
	$query = $CONFIG['dbconn'][0]->prepare('
										...
										WHERE ' . $CONFIG['db'][0]['prefix'] . 'sys_terms_be_uni.id_count = (:count)
	...
	$handle = fopen($CONFIG['system']['directoryRoot'] . $CONFIG['system']['pathAdmin'] . 'i18n/' . $filename, 'w');
