...
$out['filename'] = 'Assets-per-Countries_' . $now2 . '.xlsx';
...
$objPHPExcel->getActiveSheet()->getColumnDimension($aCols[$col])->setWidth(30);
$objPHPExcel->getActiveSheet()->getStyle($aCols[$col].$row)->getFont()->setBold(true);
$objPHPExcel->getActiveSheet()->getCell($aCols[$col].$row)->setValueExplicit('Partner', PHPExcel_Cell_DataType::TYPE_STRING);
$col++;
...
foreach($rowsT as $rowT){
	...
	$rowT['bsd'] = $rowT['parent_program_name'];
	//if(strtolower($rowT['bsd_silver']) == 'yes' || strtolower($rowT['bsd_gold']) == 'yes' || strtolower($rowT['bsd_diamond']) == 'yes') $rowT['bsd'] = 'yes';
