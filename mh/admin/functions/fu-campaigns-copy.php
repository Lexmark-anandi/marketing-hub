...
$queryI = $CONFIG['dbconn'][0]->prepare('
									INSERT INTO ' . $CONFIG['db'][0]['prefix'] . '_campaigns_
...
$queryI->execute(); 
$aNewId['campaigns'][$CONFIG['page']['id_data']] = $CONFIG['dbconn'][0]->lastInsertId();

$pkey = 'id_camp_data';
$id = 'id_campid';
foreach($aNewId['campaigns'] as $idOld => $idNew){
	// copy data
	foreach($aTabs as $tab){
		$table = $CONFIG['db'][0]['prefix'] . '_campaigns_' . $tab;
		...
		foreach($rows as $row){
			...
			$qry .= 'UPDATE tmp SET ' . $id . ' = ' . $aNewId['campaigns'][$idOld] . '; ';
...
foreach($aNewId['campaigns'] as $idOld => $idNew){
	$queryT = $CONFIG['dbconn'][0]->prepare('
										...
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_templates_uni.id_campid = (:id)
...
foreach($aNewId['campaigns'] as $idOld => $idNew){
	// copy data
	$table = $CONFIG['db'][0]['prefix'] . '_campaigns2countries_';
...
foreach($aNewId['campaigns'] as $idOld => $idNew){
	// copy data
	$table = $CONFIG['db'][0]['prefix'] . '_campaigns2products_';
...
echo $aNewId['campaigns'][$CONFIG['page']['id_data']];
