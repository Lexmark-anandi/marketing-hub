...
$specialFilter = '';
if($table == 'sys_countries_uni') $specialFilter = 'AND ' . $CONFIG['db'][0]['prefix'] . $table . '.id_countid IN (' . implode(',', array_keys($CONFIG['user']['countries'])) . ')';


$query = $CONFIG['dbconn'][0]->prepare('
									SELECT ' . $CONFIG['db'][0]['prefix'] . $table . '.' . $primary . ' AS id,
										' . $CONFIG['db'][0]['prefix'] . $table . '.' . $aArgs['data']['c'] . ' AS term
