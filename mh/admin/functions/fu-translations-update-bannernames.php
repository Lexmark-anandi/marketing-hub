...
$aBannernames = json_decode($varSQL['bannernames'], true);

foreach($aBannernames as $bfid => $bannername){
	$query = $CONFIG['dbconn'][0]->prepare('
										UPDATE ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_loc SET
											bannername = (:bannername)
										
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_loc.id_bfid = (:id)
											AND ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_loc.id_count = (:count)
											AND ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_loc.id_lang = (:lang)
										LIMIT 1
										');
	$query->bindValue(':id', $bfid, PDO::PARAM_INT);
	...
	$query->bindValue(':bannername', $bannername, PDO::PARAM_STR);
	$query->execute();

	$query = $CONFIG['dbconn'][0]->prepare('
										UPDATE ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_uni SET
											bannername = (:bannername)
										
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_uni.id_bfid = (:id)
											AND ' . $CONFIG['db'][0]['prefix'] . '_bannerformats_uni.id_count = (:count)
...
if(isset($varSQL['promid'])){
	$query = $CONFIG['dbconn'][0]->prepare('
										UPDATE ' . $CONFIG['db'][0]['prefix'] . '_promotions_loc SET
											title = (:title)
										
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_promotions_loc.id_promid = (:id)
											AND ' . $CONFIG['db'][0]['prefix'] . '_promotions_loc.id_count = (:count)
											AND ' . $CONFIG['db'][0]['prefix'] . '_promotions_loc.id_lang = (:lang)
										LIMIT 1
										');
	$query->bindValue(':id', $varSQL['promid'], PDO::PARAM_INT);
	$query->bindValue(':count', $CONFIG['settings']['selectCountry'], PDO::PARAM_INT);
	$query->bindValue(':lang', $CONFIG['settings']['selectLanguage'], PDO::PARAM_INT);
	$query->bindValue(':title', $varSQL['promotitle'], PDO::PARAM_STR);
	$query->execute();
	...
	$query = $CONFIG['dbconn'][0]->prepare('
										...
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_promotions_uni.id_promid = (:id)
											AND ' . $CONFIG['db'][0]['prefix'] . '_promotions_uni.id_count = (:count)
...
if(isset($varSQL['campid'])){
	$query = $CONFIG['dbconn'][0]->prepare('
										UPDATE ' . $CONFIG['db'][0]['prefix'] . '_campaigns_loc SET
											title = (:title)
										
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_campaigns_loc.id_campid = (:id)
											AND ' . $CONFIG['db'][0]['prefix'] . '_campaigns_loc.id_count = (:count)
											AND ' . $CONFIG['db'][0]['prefix'] . '_campaigns_loc.id_lang = (:lang)
										LIMIT 1
										');
	$query->bindValue(':id', $varSQL['campid'], PDO::PARAM_INT);
	$query->bindValue(':count', $CONFIG['settings']['selectCountry'], PDO::PARAM_INT);
	$query->bindValue(':lang', $CONFIG['settings']['selectLanguage'], PDO::PARAM_INT);
	$query->bindValue(':title', $varSQL['campaigntitle'], PDO::PARAM_STR);
	$query->execute();
	...
	$query = $CONFIG['dbconn'][0]->prepare('
										...
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_campaigns_uni.id_campid = (:id)
											AND ' . $CONFIG['db'][0]['prefix'] . '_campaigns_uni.id_count = (:count)
...
//	$queryB->bindValue(':nul', 0, PDO::PARAM_INT);
//	$queryB->execute();
//	$rowsB = $queryB->fetchAll(PDO::FETCH_ASSOC);
//	$numB = $queryB->rowCount();
//	foreach($rowsB as $rowB){
//		$pages = ($rowB['animated'] == 1) ? 3 : 1;
//
//		for($i=1; $i <= $pages; $i++){
////			$bannername = 'banner_original_' . $rowB['id'] . '_' . $i;
////
////			if(isset($aArgsSave['aData'][$bannername][0])){
////			//echo $bannername;
////				$aArgsSave['aData']['file_original'][0] = $aArgsSave['aData'][$bannername][0];
////				
////				
...
////////							$aDimension = array('mediabox' => $aMediaBox, 'cropbox' => $aCropBox, 'trimbox' => $aTrimBox);
//////				
...
////							$queryP1->bindValue(':id_bfid', $rowB['id'], PDO::PARAM_INT);
////							$queryP1->execute();
...
////							$aArgsN['n_' . $tpid]['data']['id_bfid'] = $rowB['id'];
...
////									if($field != $aArgsSaveN['n_' . $tpid]['primarykey'] && $field != 'id_tempid' && $field != 'file_original' && $field != 'page'){
////										$col .= ', ' . $field;
////										$val .= ', :' . $field . '';
////										$upd .= $field.' = (:'.$field.'), ' ;
////									}
...
//////										$col .= ', ' . $field;
//////										$val .= ', "' . $aArgsSaveN['n_' . $tpid]['aData'][$field] . '"';
//////										$upd .= $field.' = "'.$aArgsSaveN['n_' . $tpid]['aData'][$field].'", ' ;
//////									}
//////								}
//////							}
////		
