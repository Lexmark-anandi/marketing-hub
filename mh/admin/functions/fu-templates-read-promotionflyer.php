...
$aArgs['fields']['file']['specsheet_original'] = array('pf' => '0', 'thumbnail' => '0', 'functions' => array(22), 'gridheight' => 'fold', 'type' => 'single');
...
$rowsDb[0]['id_promid'] = ($CONFIG['page']['id_mod_parent'] == 113) ? $CONFIG['page']['id_data_parent'] : 0;
$rowsDb[0]['id_campid'] = ($CONFIG['page']['id_mod_parent'] == 116) ? $CONFIG['page']['id_data_parent'] : 0;
...
############
// read local versions
$aLocalVersionsTmp = array(array(0,0,0));
if(!in_array(array($CONFIG['settings']['formCountry'],$CONFIG['settings']['formLanguage'],$CONFIG['settings']['formDevice']), $aLocalVersionsTmp)) array_push($aLocalVersionsTmp, array($CONFIG['settings']['formCountry'],$CONFIG['settings']['formLanguage'],$CONFIG['settings']['formDevice'])); 
foreach($aLocalVersionsTmp as $aVersion){
	...
	if($num == 0){
		...
		if($numD > 0){
			...
			$queryN = $CONFIG['dbconn'][0]->prepare('
												SELECT 
													' . $CONFIG['db'][0]['prefix'] . '_templates2countries_.id_count2lang
			...
			$queryN = $CONFIG['dbconn'][0]->prepare('
												SELECT 
													...
													' . $CONFIG['db'][0]['prefix'] . '_templatespages_uni.id_cssid,
													' . $CONFIG['db'][0]['prefix'] . '_templatespages_uni.id_cbid,
													...
													' . $CONFIG['db'][0]['prefix'] . '_templatespages_uni.page_duration,
...
//				$rowsD[0]['id_bfid'] = $rowsN[0]['id_bfid'];
//				$rowsD[0]['id_cssid'] = $rowsN[0]['id_cssid'];
				$rowsD[0]['id_cbid'] = $rowsN[0]['id_cbid'];
//				$rowsD[0]['id_etid'] = $rowsN[0]['id_etid'];
				$rowsD[0]['kiado_code'] = $rowsN[0]['kiado_code'];
				$rowsD[0]['id_kcid'] = $rowsN[0]['id_kcid'];
				$rowsD[0]['file_original'] = array($rowsN[0]['file_original']);
				$rowsD[0]['specsheet_original'] = $rowsD[0]['file_original'];
				$rowsD[0]['page'] = $rowsN[0]['page'];
				$rowsD[0]['page_number'] = $rowsN[0]['page_number'];
				$rowsD[0]['page_duration'] = $rowsN[0]['page_duration'];
				$rowsD[0]['page_dimension'] = $rowsN[0]['page_dimension'];
			}
			
			
			###############################
			// read components
			include($CONFIG['system']['directoryRoot'] . $CONFIG['system']['pathFunctionsAdmin'] . 'fu-templates-read-components.php');
