...
$query->bindValue(':page', $varSQL['page'], PDO::PARAM_INT);
$query->execute();
...
foreach($aTCE as $tce){
	$query = $CONFIG['dbconn'][0]->prepare('
										SELECT ' . $CONFIG['db'][0]['prefix'] . '_templatesconfigurationselements_uni.id_count,
											...
											' . $CONFIG['db'][0]['prefix'] . '_templatesconfigurationselements_uni.background_color,
											' . $CONFIG['db'][0]['prefix'] . '_templatesconfigurationselements_uni.alignment,
											' . $CONFIG['db'][0]['prefix'] . '_templatesconfigurationselements_uni.verticalalignment,
	...
	foreach($rows as $row){
		...
		$queryTP = $CONFIG['dbconn'][0]->prepare('
											...
											WHERE ' . $CONFIG['db'][0]['prefix'] . '_templatespages_uni.id_count = (:id_count)
												...
												AND ' . $CONFIG['db'][0]['prefix'] . '_templatespages_uni.id_tpid = (:id_tpid)
											');
		...
		$queryTP->bindValue(':id_tpid', $varSQL['tpid'], PDO::PARAM_INT);
		$queryTP->execute();
		...
		foreach($rowsTP as $rowTP){
			if($rowTP['id_bfid'] > 0){
				if($rowTP['page'] == $varSQL['page']) $tp = $rowTP['id_tpid'];
			}else{
				$tp = $rowTP['id_tpid'];
			}
		}
		...
		if($CONFIG['user']['specifications'][14] == 8 && $CONFIG['settings']['formCountry'] == 0 && $CONFIG['settings']['formLanguage'] == 0 && $CONFIG['settings']['formDevice'] == 0){
			...
			foreach($aLocalVersions as $version){
				...
				$queryI->bindValue(':page_id', $tp . '_' . $varSQL['page'], PDO::PARAM_INT);
				$queryI->bindValue(':page', $varSQL['page'], PDO::PARAM_INT);
				...
				$queryI->bindValue(':background_color', $row['background_color'], PDO::PARAM_INT);
				$queryI->bindValue(':alignment', $row['alignment'], PDO::PARAM_STR);
				$queryI->bindValue(':verticalalignment', $row['verticalalignment'], PDO::PARAM_STR);
		...
		foreach($aLocalVersions as $version){
			...
			$page_id = 'page_' . $tp . '_' . $varSQL['page'];
			...
			if(!array_key_exists($compid, $aComponentsTempdata[$key]['pages'][$page_id])){
				...
				$aComponentsTempdata[$key]['pages'][$page_id][$compid]['pageid'] = $tp . '_' . $varSQL['page'];
				$aComponentsTempdata[$key]['pages'][$page_id][$compid]['page'] = $varSQL['page'];
				$aComponentsTempdata[$key]['pages'][$page_id][$compid]['elementtitle'] = $row['elementtitle'];
				...
				$aComponentsTempdata[$key]['pages'][$page_id][$compid]['background_color'] = $row['background_color'];
				...
				$aComponentsTempdata[$key]['pages'][$page_id][$compid]['verticalalignment'] = $row['verticalalignment'];
...
foreach($aLocalVersions as $version){
	...
	foreach($rowsTd as $rowTd){
		...
		$aTemddataComponents = json_decode($varSQL['components'], true);
		$key = $rowTd['id_count'] . '_' . $rowTd['id_lang'];
		foreach($aComponentsTempdata[$key]['pages'] as $pageid => $aComponents){
			$aTemddataComponents['pages'][$pageid] = $aComponents;
		}
		$aTemddata['components'] = json_encode($aTemddataComponents);
