...
$queryStrTPE = 'SELECT 
		...
		' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_uni.del
...
if($numTPE > 0){
	foreach($rowsTPE as $rowTPE){
		...
		if(!array_key_exists('compboxOuter_' . $rowTPE['id_tpeid'], $aTPE['pages']['page_' . $rowTPE['page_id']])){
			...
			$aTPE['pages']['page_' . $rowTPE['page_id']]['compboxOuter_' . $rowTPE['id_tpeid']]['del'] = $rowTPE['del'];
...
if($numTPE == 0){
	...
	if($numTp > 0){
		if($rowsTp[0]['id_caid'] == 3){
			...
			if($numTPE > 0){
				foreach($rowsTPE as $rowTPE){
					$queryStrTPEnew = 'SELECT 
											' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_loc.id_tpeid
										FROM ' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_loc 
										
										WHERE ' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_loc.id_count = (:id_count)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_loc.id_lang = (:id_lang)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_loc.id_dev = (:id_dev)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_loc.id_cl IN (0, ' . $CONFIG['activeSettings']['id_clid'] . ')
											AND ' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_loc.del = (:nultime)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_loc.id_tempid = (:id_tempid)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_loc.id_tpid = (:id_tpid)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_loc.id_tcid = (:id_tcid)
											AND ' . $CONFIG['db'][0]['prefix'] . '_templatespageselements_loc.elementtitle = (:elementtitle)
									';
					
					$queryTPEnew = $CONFIG['dbconn'][0]->prepare($queryStrTPEnew);
					$queryTPEnew->bindValue(':id_count', 0, PDO::PARAM_INT);
					$queryTPEnew->bindValue(':id_lang', 0, PDO::PARAM_INT);
					$queryTPEnew->bindValue(':id_dev', 0, PDO::PARAM_INT);
					$queryTPEnew->bindValue(':nultime', '0000-00-00 00:00:00', PDO::PARAM_STR);
					$queryTPEnew->bindValue(':id_tempid', $CONFIG['page']['id_data'], PDO::PARAM_INT);
					$queryTPEnew->bindValue(':id_tpid', $rowsTp[0]['id_tpid'], PDO::PARAM_INT);
					$queryTPEnew->bindValue(':id_tcid', $rowTPE['id_tcid'], PDO::PARAM_INT);
					$queryTPEnew->bindValue(':elementtitle', $rowTPE['elementtitle'], PDO::PARAM_STR);
					$queryTPEnew->execute();
					$rowsTPEnew = $queryTPEnew->fetchAll(PDO::FETCH_ASSOC);
					$numTPEnew = $queryTPEnew->rowCount();
					...
					if($numTPEnew == 0){
						...
						$queryE = $CONFIG['dbconn'][0]->prepare('
															...
															(id_tpeid, id_count, id_lang, id_dev, id_cl, restricted_all, id_tempid, id_caid, id_tpid, id_tcid, page_id, page, elementtitle, position_left, position_top, width, height, fontsize, fontcolor, fontstyle, alignment, verticalalignment, background_color, max_char, editable, content, content_transrequired, fixed, active, create_at, create_from, change_from)
															VALUES
															(:id_tpeid, :id_count, :id_lang, :id_dev, :id_cl, :restricted_all, :id_tempid, :id_caid, :id_tpid, :id_tcid, :page_id, :page, :elementtitle, :position_left, :position_top, :width, :height, :fontsize, :fontcolor, :fontstyle, :alignment, :verticalalignment, :background_color, :max_char, :editable, :content, :content_transrequired, :fixed, :active, :create_at, :create_from, :create_from)
															');
						$queryE->bindValue(':id_tpeid', $rowTPE['id_tpeid'], PDO::PARAM_INT);
						$queryE->bindValue(':id_count', 0, PDO::PARAM_INT);
						$queryE->bindValue(':id_lang', 0, PDO::PARAM_INT);
						$queryE->bindValue(':id_dev', 0, PDO::PARAM_INT);
						$queryE->bindValue(':id_cl', $CONFIG['activeSettings']['id_clid'], PDO::PARAM_INT);
						$queryE->bindValue(':restricted_all', 0, PDO::PARAM_INT);
						$queryE->bindValue(':id_tempid', $CONFIG['page']['id_data'], PDO::PARAM_INT);
						$queryE->bindValue(':id_caid', $rowsTp[0]['id_caid'], PDO::PARAM_INT);
						$queryE->bindValue(':id_tpid', $rowsTp[0]['id_tpid'], PDO::PARAM_INT);
						$queryE->bindValue(':id_tcid', $rowTPE['id_tcid'], PDO::PARAM_INT);
						$queryE->bindValue(':page_id', $rowsTp[0]['id_tpid'] . '_' . $rowTPE['page'], PDO::PARAM_STR);
						$queryE->bindValue(':page', $rowTPE['page'], PDO::PARAM_INT);
						$queryE->bindValue(':elementtitle', $rowTPE['elementtitle'], PDO::PARAM_STR);
						$queryE->bindValue(':position_left', $rowTPE['position_left'], PDO::PARAM_STR);
						$queryE->bindValue(':position_top', $rowTPE['position_top'], PDO::PARAM_STR);
						$queryE->bindValue(':width', $rowTPE['width'], PDO::PARAM_STR);
						$queryE->bindValue(':height', $rowTPE['height'], PDO::PARAM_STR);
						$queryE->bindValue(':fontsize', $rowTPE['fontsize'], PDO::PARAM_STR);
						$queryE->bindValue(':fontcolor', $rowTPE['fontcolor'], PDO::PARAM_STR);
						$queryE->bindValue(':fontstyle', $rowTPE['fontstyle'], PDO::PARAM_STR);
						$queryE->bindValue(':alignment', $rowTPE['alignment'], PDO::PARAM_STR);
						$queryE->bindValue(':verticalalignment', $rowTPE['verticalalignment'], PDO::PARAM_STR);
						$queryE->bindValue(':background_color', $rowTPE['background_color'], PDO::PARAM_STR);
						$queryE->bindValue(':max_char', $rowTPE['max_char'], PDO::PARAM_INT);
						$queryE->bindValue(':editable', $rowTPE['editable'], PDO::PARAM_INT);
						$queryE->bindValue(':content', $rowTPE['content'], PDO::PARAM_STR);
						$queryE->bindValue(':content_transrequired', $rowTPE['content_transrequired'], PDO::PARAM_INT);
						$queryE->bindValue(':fixed', $rowTPE['fixed'], PDO::PARAM_INT);
						$queryE->bindValue(':active', $rowTPE['active'], PDO::PARAM_INT);
						$queryE->bindValue(':create_at', $now, PDO::PARAM_STR);
						$queryE->bindValue(':create_from', $CONFIG['user']['id_real'], PDO::PARAM_INT);
						$queryE->execute();
						...
						foreach($aLocalVersions as $aVersion){
							$queryE = $CONFIG['dbconn'][0]->prepare('
																...
																');
							$queryE->bindValue(':id_tpeid', $rowTPE['id_tpeid'], PDO::PARAM_INT);
							$queryE->bindValue(':id_count', $aVersion[0], PDO::PARAM_INT);
							$queryE->bindValue(':id_lang', $aVersion[1], PDO::PARAM_INT);
							$queryE->bindValue(':id_dev', $aVersion[2], PDO::PARAM_INT);
							$queryE->bindValue(':id_cl', $CONFIG['activeSettings']['id_clid'], PDO::PARAM_INT);
							$queryE->bindValue(':restricted_all', 0, PDO::PARAM_INT);
							$queryE->bindValue(':id_tempid', $CONFIG['page']['id_data'], PDO::PARAM_INT);
							$queryE->bindValue(':id_caid', $rowsTp[0]['id_caid'], PDO::PARAM_INT);
						...
						$rowTPE['id_tpeid'] = $rowsTPEnew[0]['id_tpeid'];
					}
					...
					if(!array_key_exists('compboxOuter_' . $rowTPE['id_tpeid'], $aTPE['pages']['page_' . $rowTPE['page_id']])){
						...
						$aTPE['pages']['page_' . $rowTPE['page_id']]['compboxOuter_' . $rowTPE['id_tpeid']]['verticalalignment'] = $rowTPE['verticalalignment'];
						$aTPE['pages']['page_' . $rowTPE['page_id']]['compboxOuter_' . $rowTPE['id_tpeid']]['fixed'] = $rowTPE['fixed'];
						$aTPE['pages']['page_' . $rowTPE['page_id']]['compboxOuter_' . $rowTPE['id_tpeid']]['del'] = '0000-00-00 00:00:00';
